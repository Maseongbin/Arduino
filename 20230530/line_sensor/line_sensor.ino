...
int read_line_sensor(void)
{
  ...
  int sum = 0; // line sensor에서 불이 들어온 sensor 갯수
   for(i=0;i<5;i++)
  {
    line_sensor[i] = 1 - digitalRead(line_sensor_pin[i]); // 흰색이면 1, 검정색이면 0
    sum += line_sensor[i];
...
void line_following(int line_index)
{
  switch(line_index)
  {
    case -4: motor_L_control(-10); motor_R_control(120); 
           break;
    case -3:
           break;
    case -2:
    ...
    case -1:
           break;
    case  0: motor_L_control(80); motor_R_control(100); 
           break;
    case  1:
           break;
    case  2:
