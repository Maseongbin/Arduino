...
void vision_line_control(int base_speed, double l_c)
{
  ...
  motor_control_L(base_speed * 2.0 - pwm_control_value);
  motor_control_R(base_speed * 0.9 + pwm_control_value);
}
...
void VLC_move(void)  
{
  ...
  cx_int = (int)cx;
  read_line_camera();
  threshold_line_image(40);
  cx = line_centroid();
  vision_line_control(70, cx);
