...
double kp_vision = 0.1; //0.5
double ki_vision = 0.3; //0.3
double kd_vision = 1.0; //3.0
double error     = 0.0;
...
#define THRESHOLD_ANGLE1 20 //15
#define THRESHOLD_ANGLE2 11  //7

LSM303 compass;

float heading_angle        = 0.0;
float init_heading_        = 0.0;   // 초기 방향
...
double pulst_to_distance_left = 0.75 / 180.2;
double pulst_to_distance_right = 0.75 / 179.4;
...
void read_imu_sensor(void)
{
  ...
  delay(50);  // Delay for a short time (adjust as needed) to allow for more accurate readings
  compass.read();
...
double PID_control(double line_center)
{
  ...
  if (pwm_value >=  200) pwm_value = 255;
  if (pwm_value <= -200) pwm_value = -255;
...
void vision_line_control(int base_speed, double l_c)
{
  ...
  int temp_data=0;
  temp_data =(base_speed + pwm_control_value)*1.0;
...
void VLC_move(void)  // 비전 라인 컨트롤
{
  ...
  cx_int = (int)cx;
  read_line_camera();
  threshold_line_image(40);  // 값 조정
  cx = line_centroid();
  vision_line_control(100, cx);
...
void loop()
{
  ...
  read_imu_sensor();
  Serial.print("IMU Heading Angle: ");
  ...
  switch (state_vlc) 
  {
    ...
    case 1: // Rotate to reach the desired IMU angle
      ...
      read_imu_sensor();
      target_heading_angle = 11;

      if (abs(heading_angle - target_heading_angle) > 0.5)
      {
        imu_rotation();
