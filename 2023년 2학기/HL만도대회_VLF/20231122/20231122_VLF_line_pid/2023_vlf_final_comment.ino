...
void find_line_center(void)
{
  ...
  if (LineSensor_Data_Adaption[Line_Center] != 255) // Line이 2개 검출 될 때
  {
...
void read_ultrasonic_sensor(void)
{
  ...
  /*
    ...
    if (  UltrasonicSensorData[2] == 0)
    {
      UltrasonicSensorData[2] = MAX_DISTANCE;
    }*/
}
...
{
  error_curve   = Line_Center - NPIXELS / 2 + OFFSET ; // 카메라 보정 방향 반대시 전체에 -1 곱하기
...
{
  sonar_data = UltrasonicSensorData[2] - UltrasonicSensorData[1]; // 오른쪽소나 - 왼쪽 소나
