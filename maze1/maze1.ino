...
void motoer_A_control(int direction_a, int motor_speed_a) //모터 A의 방향(direction)과 속도(speed) 제어
{
  if(direction_a == HIGH)
  {
...
void motoer_B_control(int direction_b, int motor_speed_b) //모터 A의 방향(direction)과 속도(speed) 제어
{
  if(direction_b == HIGH)
  {
...
void loop() 
{
 ...
 /* 
  ...
  motoer_A_control(LOW,0); //오른쪽 정지
  motoer_B_control(HIGH,0); //왼쪽은 정지
  delay(500);                //일정한 시간 동안 정지
    
  //180도 회전 반시계 방향으로 회전
  Serial.println("Rotate CCW");
  motoer_A_control(LOW,100); //오른쪽 전진
  motoer_B_control(HIGH,100); //왼쪽은 후진
  ...
  */
  ...
  //90도 회전 시계 방향으로 회전
  Serial.println("Rotate CCW");
  motoer_A_control(HIGH,100); //오른쪽 전진
  motoer_B_control(LOW,100); //왼쪽은 후진
  ...
  motoer_A_control(HIGH,80);
  motoer_B_control(HIGH,100);
  delay(500);
  */
  /*
  //90도 회전 반시계 방향으로 회전
